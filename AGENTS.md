...
SvelteKit fullstack (frontend Y backend) para cotizadores de ventanas, desplegado en Cloudflare Pages + D1 (SQLite) con **Prisma** (`@prisma/adapter-d1`, driver adapter). Comentarios/errores/README en español. La rama de trabajo es `main` (`origin` = fork Skyrdow, `client-repo` = upstream termoacusticos).

## Comandos (usa **npm** y Node.js; Bun falla en este proyecto)

- `npm ci` — tras actualizar dependencias: `npx prisma generate` (el cliente NO está en repo). El script `build` ejecuta sync y generate explícitamente, porque algunos entornos CI bloquean scripts `postinstall`.
- Verificación: `npm run format` → `npm run lint` → `npm run check` → `npm run build`
- Dev local con DB: `npm run devw` (= `vite build` con adapter-cloudflare + `wrangler pages dev .svelte-kit/cloudflare`)
- Dev sólo UI: `npm run dev` (sin DB, ver gotchas)
- Tests: `npm test` corre Vitest; `src/lib/services/calculadora.test.ts` cubre el cálculo simulando los repositories. Vitest puede mostrar un aviso de cierre demorado después de pasar; confirmar que el proceso finalice con código 0.
...
- Schema: `prisma/schema.prisma` → SQL: `migrations/0001_create_tables.sql` (tablas PascalCase: `"Usuario"`, `"Presupuesto"`, ...)
  - regenerar tras tocar schema: `npx prisma migrate diff --from-empty --to-schema-datamodel ./prisma/schema.prisma --script --output migrations/0001_create_tables.sql`
  - aplicar: `npx wrangler d1 migrations apply pingeso-demo --local` (o `--remote`; wrangler ya está logueado; `pingeso-demo` figura en `wrangler.toml`)
  - seed: `npx wrangler d1 execute pingeso-demo --local|--remote --file src/lib/sql/insert.sql` (perfiles, tipos, materiales, constantes, etc.)
...
- **wrangler debe correr bajo Node**: con wrangler bajo runtime de Bun, workerd acepta el socket pero **ninguna petición responde** (ni estática). Ejecutar los comandos con Node/npm.
- **`JWT_SECRET` es obligatorio en runtime**: `src/lib/index.ts` lo lee de `$env/dynamic/private` al validar o firmar JWT. No se necesita durante la compilación. Para `npm run dev`, mantenerlo en `.env`; Wrangler (`devw`/Pages dev) lee bindings locales desde `.dev.vars`. En Cloudflare Pages configurarlo como secret de Runtime para los entornos Production y Preview que correspondan. `.dev.vars*` está gitignored.
- **`npm run dev` no tiene DB**: `platform` es `undefined` → `getDB()` retorna `err` → todo `/api/*` responde 400 `falló la conexión con la db`. Con `devw` (wrangler) sí funciona — todo el flujo de auth/DB probarlo con `devw`.
- `npm run lint`, `npm run check`, `npm run build` y `npm test` pasan actualmente. `lint` = `prettier --check . && eslint .`; usa `npm run format` para corregir formato.
...
- Iconos: plugin `@iconify/tailwind` (`<span class="iconify mdi--account-alert-outline">`); set nuevo: `npm install -D @iconify-json/<set>` + `addIconSelectors([...])` en `tailwind.config.js`.
...
- README: Ante conflicto, manda el código.
